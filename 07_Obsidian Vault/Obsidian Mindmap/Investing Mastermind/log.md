...
## [2026-09-04] system | R6 Suchraum-Komplement — Tool `spec_inventory_diff.py`, `UMSTRUKTURIERUNG-2027.md` zurückgezogen, `PIPELINE.md`-Kollision abgegrenzt (scoring-neutral, system-zustand)

**Event-Typ:** System-Zustand. Erste Prüfrunde, die den Suchraum nicht argumentativ, sondern **mechanisch** bildet.

**Ausgangslage.** Sechs Prüfrunden (R1–R5 + Codex-C5) lagen auf der Spec, und jede fand Befunde außerhalb des Suchraums der Vorrunde. Die Frage war, wie man aus dieser Reihe herauskommt, statt sie fortzusetzen. Antwort: nicht noch eine Runde desselben Typs, sondern ein deterministischer Mengen-Diff — Repo-Inventar minus die Dateien, die die Spec namentlich nennt.

**Tool: `03_Tools/spec_inventory_diff.py`.** Grundmenge `git ls-files` + untracked-nicht-ignoriert, gefiltert auf regelwerk-fähige Endungen, Ausschlüsse **deklariert und mitgedruckt** (Wiki-Korpus 245 · Cache/Fixtures 56 · Analysen 19 · Archiv 10 · Templates 1). Vier Listen: **A** vom Agenten per Regel gelesen und ungenannt · **B** Rest · **C** mehrdeutiger Basename · **D** Spec nennt, Repo hat nicht. Exit 1 bei nicht-leerem A.

**Abnahmekriterium vorab definiert — und es schlug beim ersten Lauf fehl.** Das Tool musste den bekannten Fehlbefund reproduzieren: Lauf gegen Spec v1.6 (`e88f0d7`) muss `00_Core/STATE.md` in Liste A zeigen. Tat es nicht. Ursache: das Matching war lenient (Basename **oder** Stem), und v1.6 enthält das bloße Wort `STATE` in einer Aufzählung — damit galt die Datei als „erwähnt". **Genau der Denkfehler, den das Tool aufdecken soll: in einer Liste genannt ≠ als Quelle behandelt.** Match jetzt strikt auf den Basename; endungslos nur für `.xlsx`, weil die Spec sie durchgängig so zitiert. Danach: v1.6 → `STATE.md` in A (8 Treffer), v1.7 → nicht mehr. **Lehre: bei Coverage-Werkzeugen ist großzügiges Matching nicht Kulanz, sondern Blindheit — und ein Abnahmetest gegen den Ist-Stand hätte den Bug nie gezeigt. Es braucht einen bekannten historischen Fehlbefund als Fixture (`git show <SHA>:<pfad>`).**

**Das Tool hatte dieselbe Fehlerklasse, die es sucht.** Die Endungsliste ist selbst ein Suchraum nach Datei-Sorte: `.pdf` (34), `.log` (3), `.toml`, `.ps1`, `.docx` fielen komplett durch. `.toml`/`.ps1`/`.txt` nachgezogen (3 reale Dateien, Suchraum 182 → 185); der Rest bleibt bewusst draußen — aber das Script **druckt die Rest-Lücke jetzt aus**, statt sie vorauszusetzen. Das ist die eigentliche Kur: Lücken sichtbar machen, nicht stopfen.

**Fund 1 — `00_Core/UMSTRUKTURIERUNG-2027.md` war eine nicht inventarisierte Regelwerk-Quelle.** 149 Zeilen mit 60/35/5, 3-Tier-Basis 40/32/18 und der Zeile „FLAG bleibt heilig — geflaggte Titel = 0 € trotz Tier (**Override-frei**)", die dem Wortlaut nach mit dem Escape-Hatch §3.3 der Spec kollidiert. Strukturgleich mit `KONTEXT.md` (§2.4) und `STATE.md` (§2.4b) — also die **fünfte** Quelle derselben Klasse. **Owner-Entscheidung: zurückgezogen**, obsolet durch vollzogenen Depotübertrag auf Scalable + MCP/CLI-Implementierung.

**Bemerkenswert am Fundweg:** Die Datei landete in Liste **B**, nicht A — ihr Basename steht in keiner der Tier-A-Quellen (`CLAUDE.md`, `.pre-commit-config.yaml`, `para18_sync_reminder.py`), sie wird über `STATE.md` erreicht. **Tier A erfasst nur Erst-Ordnungs-Lesewege.** Wer nur A abgearbeitet hätte, wäre am stärksten Treffer der Runde vorbeigelaufen.

**Bereinigung — 3 Pointer, nicht 56 Dateien.** `grep` fand 56 Dateien mit „Umstrukturierung", aber nur **drei** sind live Datei-Links (`STATE.md` Z. 15 · `CORE-MEMORY.md` Z. 27 · `KONTEXT.md` Z. 91). Der Rest sind ~89 Nennungen des **Ereignisnamens** ohne Dateibezug — gültige Provenienz für Score-/Tier-Entscheidungen, das Ereignis hat stattgefunden. **Bewusst nicht angetastet:** der Sync-Set-Eintrag vom 12.06. in diesem Log (er protokolliert, was damals synchronisiert wurde — Umschreiben wäre Fälschung) und `score_history.jsonl` (append-only, hook-validiert). Ein naiver `sed`-Sweep hätte beide mitgerissen. Die drei Pointer tragen jetzt einen **Rückzugs-Vermerk** statt stiller Löschung, damit der Weg zu `git log` erhalten bleibt.

**Inhalt vs. Verweis.** Zwei der drei Stellen delegierten Substanz, nicht nur einen Link — `KONTEXT.md` §4b verwies für den „vollständigen Transaktionsplan + Timing" dorthin. Gegen den Live-Stand geprüft: der **Transaktionsplan** ist vollzogen, **Phase B nicht** — AVD-Start Jan 2027, Re-Derive-Gate (Combined-View mit EXUS) und AVD/Riester-Übertrag stehen weiter offen und sind in §4b oben geregelt. Der Vermerk sagt das jetzt explizit, statt eine Aussage ohne Deckung stehen zu lassen.

**Fund 2 — `PIPELINE.md` existiert zweimal.** `00_Core/PIPELINE.md` (93 Z.) ist der Work-SSoT; `01_Skills/dynastie-depot/PIPELINE.md` (293 Z.) ist die Analyse-Pipeline-Doku des Skills und trägt das 3-Tier-Ratenmodell. Owner-Klarstellung festgehalten in **beiden** Kopfzeilen (am Ort der Verwechslung) und in Spec §10, wo der Pfad jetzt qualifiziert ist. Die Skill-Datei ist ausdrücklich **kein** §18-Sync-Ziel der Klasse „Pipeline-Item".
