---
tags: [satellit, aktiv, defcon-2]
ticker: TMO
name: Thermo Fisher Scientific
sektor: Life-Science / Instrumente
ersatz: ZTS
score_aktuell: 63
defcon: 2
flag: "keins"
sparrate: "Halbe Rate 16,76€ (D2, Gewicht 0.5)"
letzteAnalyse: 2026-04-17
score_valid_until: 2026-10-14
naechsterTrigger: "2026-04-23 — Q1 2026 Earnings"
scoring_notiz_v37: "v3.7: Fix 2 (OpM) + Fix 3 (Sell-Ratio) drücken Score 67→63, D3→D2. ZTS-Vorbereitung bleibt aktiv."
related_concepts: "[[5J-Fundamental-Fenster]], [[FCF-Primacy]], [[Moat-Taxonomie-Morningstar]]"
---

# TMO — Thermo Fisher Scientific

> **DEFCON 🟡 3 | Score 67/100 | Kein FLAG**
> Sparrate: 50% Sockelbetrag

## Aktuelle Lage (Stand: 07.04.2026)

Score von 65 auf 67 gestiegen (Re-Analyse v3.4). Verbesserungen durch Kursrückgang (-24% vom ATH) und Clario-Schuldenabbau. Kernprobleme bleiben: Goodwill $49.4B (44.8% der Assets), ROIC 2.6% (quartalsweise), FCF -13.4% YoY.

## Schwächen

- ROIC **2.6%** vs. WACC ~10% → strukturell unter Kapitalkosten
- Goodwill durch Akquisitionen verzerrt GAAP-ROIC dauerhaft
- FCF rückläufig — Clario-Integration noch nicht voll verdaut
- Kurs unter 200MA

## Stärken (erhalten)

- Wide Moat bestätigt (Life-Science-Infrastruktur, Switching Costs)
- Net Debt/EBITDA 3.6x → 2.57x ✅ (Schuldenabbau läuft)
- Fwd P/E 27x → 19.9x ✅ (Kursrückgang verbessert Bewertung)
- P/FCF 35x → 29.2x ✅

## Entscheidungsbaum nach Q1 Earnings (23.04.2026)

- FCF >$7.3B (Yield >4%) + ROIC Aufwärtstrend → **DEFCON 4** möglich
- FCF weiter rückläufig ODER Moat-Downgrade → **ZTS-Aktivierung prüfen**
- Score <50 → DEFCON 1 → sofortiger Ersatz-Prozess

## Verlinkungen

- [[DEFCON-System]]
- [[ROIC-vs-WACC]]
- [[ZTS]] — Ersatz-Kandidat (Ersatzbank)

## Analyse-Historie

| Datum | Score | DEFCON | Ereignis |
|-------|-------|--------|---------|
| 02.04.2026 | 65 | 🟡 3 | Erst-Analyse v3.4 — Clario-Watch gesetzt |
| 07.04.2026 | 67 | 🟡 3 | Re-Analyse — Verbesserungen bestätigt |

## Wissenschaftliche Basis
- [[5J-Fundamental-Fenster]] — 5J-Perspektive als Pflichtrahmen für alle Fundamentaldaten
- [[FCF-Primacy]] — FCF-Yield und forward P/E als primäre Bewertungsanker; trailing P/E: nur Kontext
- [[Moat-Taxonomie-Morningstar]] — Moat-Prüfung nach 8-Quellen-Schema (Wide/Narrow/None)
- [[Wissenschaftliche-Fundierung-DEFCON]] — 7-Befunde-Matrix: wissenschaftliche Validierung des DEFCON-Systems
