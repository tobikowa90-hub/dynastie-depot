---
tags: [satellit, aktiv, defcon-2]
ticker: TMO
name: Thermo Fisher Scientific
sektor: Life-Science / Instrumente
ersatz: ZTS
score_aktuell: 64
defcon: 2
flag: "keins (fcf_trend_neg schema-getriggert → struktureller Disclosure, nicht aktiviert)"
sparrate: "Halbe Rate 17,81€ (D2, Gewicht 0.5)"
letzteAnalyse: 2026-04-18
score_valid_until: 2026-10-15
naechsterTrigger: "2026-04-23 — Q1 FY26 Earnings (D2-Entscheidung + fcf_trend_neg Resolve-Gate)"
scoring_notiz_v37: "v3.7 Forward-Vollanalyse 18.04.: Algebra-Projektion 63 empirisch bestätigt (±1, Score 64). ROIC bereinigt 17,18% vs WACC 10,44% = +6,74pp (Regel-4-Gating: GW/Assets 44,74% ≥30% → Goodwill-Ausnahme gerechtfertigt, Invested Capital bereinigt = 43,430M). fcf_trend_neg strukturell ausgesetzt per Advisor-Review (Option B): WC-Noise + 4J-Plateau + OpInc +5,1%."
related_concepts: "[[5J-Fundamental-Fenster]], [[FCF-Primacy]], [[Moat-Taxonomie-Morningstar]], [[DEFCON-System]]"
updated: 2026-04-18
---

# TMO — Thermo Fisher Scientific

> **DEFCON 🟠 2 | Score 64/100 (v3.7 Forward) | struktureller fcf_trend_neg Disclosure (kein FLAG aktiv)**
> Sparrate: 17,81€ (D2, Gewicht 0.5)

## Aktuelle Lage (Stand: 18.04.2026 — v3.7 Forward-Vollanalyse)

Score 64 (Algebra-Projektion 63 ±1 empirisch bestätigt). Kern-Neuigkeit: **ROIC-Regel-4-Gating greift** — GW/Assets 44,74% ≥30% rechtfertigt die Goodwill-Ausnahme (Invested Capital bereinigt = 92,792M − 49,362M = 43,430M → ROIC bereinigt **17,18% vs WACC 10,44% = +6,74pp Spread**). Die historisch zitierte "2,6% ROIC" war die unbereinigte Quartalsversion.

**fcf_trend_neg FLAG-Entscheidung (18.04., via Advisor — Option B struktureller Disclosure):**

Schema-Signal war getriggert: FCF FY25 $6,293M vs FY24 $7,267M = **−13,4% YoY**; CapEx +8,9% YoY. Der Advisor verwarf mechanischen Trigger mit drei strukturellen Gegenerklärungen:

| Einwand | Evidenz |
|---|---|
| (a) WC-Noise | WC-Delta FY25 −$1,766M vs −$334M FY24 (Δ **−$1,432M**) erklärt FCF-Rückgang −$974M **überproportional** — operativer Cashflow-Kern intakt |
| (b) 4J-Plateau | FY22-25 FCF $6,911M → $6,927M → $7,267M → $6,293M → Schwankungen um Plateau, **kein Mehrjahres-Abwärtstrend** |
| (c) Profitabilität intakt | Operating Income FY25 **+5,1% YoY** ($8,110M vs $7,717M) — Margin-Story lebt |

**Resolve-Gate:** Q1 FY26 Earnings 23.04.2026 — WC-Unwind + FCF-Recovery bestätigt → Disclosure bleibt; fehlende Reversibilität → fcf_trend_neg-Trigger nachzutragen.

## DEFCON v3.7 Analyse (18.04.2026)

| Block | Score | Details |
|-------|-------|---------|
| Fundamentals | 30/50 | Fwd P/E 19,43 → 6 · P/FCF 31,10 → 1 (QT-cap Wide+22-35) · Bilanz 6 (Net Debt/EBITDA 2,57x / CR 1,89 / GW 44,74%) · CapEx/OCF 19,51% → 7 · **ROIC 6 (bereinigt 17,18% vs WACC 10,44% = +6,74pp, Regel-4-Gating greift)** · FCF-Y 3,22% → 3 · OpM 21,17% → 1 |
| Moat | 18/20 | Wide (Intangibles + Switching Costs + Scale). GM-Trend flat (40,9% FY25 / 41,3% FY24 / 39,9% FY23) → 0. Kein Pricing-Power-Bonus. |
| Technicals | 6/10 | ATH-Dist 3 (−18% vs $644 52w-High) · RelStärke 0 · Trend 3 (Kurs $526,60 knapp über 200MA $523,72) |
| Insider | 4/10 | Net-Buy 0 · Ownership 1 (<1% strikt) · Kein_20M 3 |
| Sentiment | 6/10 | SB 4 (strong_buy) · Sell 1 · PT-Up 2 (+23% zu $648 Median) · PT-Disp −1 (35,5% Range $520–$750) |

## Stärken

- Wide Moat bestätigt (Life-Science-Instrumente + Reagenzien-Switching-Costs + Scale)
- **ROIC bereinigt 17,2% vs WACC 10,4%** (Regel-4-Gating greift dank GW/Assets 44,7%)
- Net Debt/EBITDA 2,57x nach Schuldenabbau (von 3,6x)
- Operating Income +5,1% YoY = Profitabilität intakt

## Schwächen

- FCF-Volatilität durch WC-Dynamik (Clario-Integration noch nicht voll ausgeschwungen)
- GAAP-ROIC 8,04% unterzeichnet ökonomischen Return (Goodwill-Verzerrung)
- Fwd P/E 19,4 nach −18% vom ATH = technischer Abwärtstrend setzt sich fort
- PT-Dispersion 35,5% = erhöhte Analysten-Unsicherheit

## Entscheidungsbaum nach Q1 FY26 Earnings (23.04.2026)

- FCF-Recovery + WC-Unwind + ROIC-Aufwärtstrend → **DEFCON 3** möglich, Disclosure bleibt Notiz
- FCF weiter rückläufig + WC-Delta persistent → **DEFCON 2 bleibt**
- fcf_trend_neg strukturell bestätigt (fehlende WC-Reversibilität) → **FLAG nachzutragen**
- Score <50 → DEFCON 1 → ZTS-Aktivierung (muss Score ≥80 ohne FLAG haben)

## Verlinkungen

- [[DEFCON-System]]
- [[ROIC-vs-WACC]]
- [[ZTS]] — Ersatz-Kandidat (Ersatzbank, Vorbereitung aktiv)

## Analyse-Historie

| Datum | Score | DEFCON | Ereignis |
|-------|-------|--------|---------|
| 02.04.2026 | 65 | 🟡 3 | Erst-Analyse v3.4 — Clario-Watch gesetzt |
| 07.04.2026 | 67 | 🟡 3 | Re-Analyse — Verbesserungen bestätigt |
| 17.04.2026 | 63 | 🟠 2 | v3.7 Algebra-Projektion (Fix-2 OpM + Fix-3 Sell-Ratio, -4 Pt.) — D3→D2 |
| 18.04.2026 | **64** | 🟠 2 | **v3.7 Forward-Vollanalyse** — ROIC-Regel-4-Gating korrekt angewendet (+6,74pp Spread). fcf_trend_neg schema-getriggert, struktureller Disclosure (Advisor Option B). Record `2026-04-18_TMO_vollanalyse`. |

## Wissenschaftliche Basis

- [[5J-Fundamental-Fenster]]
- [[FCF-Primacy]]
- [[Moat-Taxonomie-Morningstar]]
- [[Wissenschaftliche-Fundierung-DEFCON]]

## Factor-Exposure (Aghassi 2023)

Einordnung nach [[Factor-Investing-Framework]]. Strikt dokumentativ, keine Score-Wirkung.

- **Value:** moderat — Fwd P/E leicht über 5J-Median, Quality-Premium gerechtfertigt
- **Quality:** stark — ROIC >20%, Moat wide (Life-Sciences-Instrumentierung), QMJ-Profil
- **Momentum:** schwach — Rücksetzer 2025, Q1 23.04. FLAG-Resolve-Gate entscheidet
- **Defensive:** moderat — Beta moderat, zyklisch via Biotech-Kunden
- **Investment:** **schema-getriggert (nicht aktiviert)** — fcf_trend_neg FY25 WC-Noise; Schema-Watch, kein aktiver FLAG. §30 daher NICHT automatisch pflicht (siehe [[Factor-Information-Decay]])

Quellen: [[Aghassi-2023-Fact-Fiction]], [[QMJ-Faktor]], [[Flint-Vermaak-2021-Decay]]
