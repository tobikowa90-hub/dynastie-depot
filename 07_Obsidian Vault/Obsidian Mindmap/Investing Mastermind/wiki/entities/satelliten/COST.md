---
tags: [satellit, aktiv, defcon-4, screener-exception]
ticker: COST
name: Costco Wholesale Corporation
sektor: Retail / Mitgliedschaftsmodell
ersatz: ""
score_aktuell: 69
defcon: 4
flag: "keins"
sparrate: "Volle Rate"
letzteAnalyse: 2026-04-17
score_valid_until: 2026-10-14
naechsterTrigger: "Q1 FY27 Earnings ~Dez 2026"
related_concepts: "[[5J-Fundamental-Fenster]], [[FCF-Primacy]], [[Moat-Taxonomie-Morningstar]]"
updated: 2026-04-17
sparrate_eur: "33,53€ (volle Rate v3.7)"
scoring_notiz_v37: "v3.7 Backtest: Score 69 bleibt stabil (Screener-Exception Membership Yield nicht von Fix 1 betroffen, OpM <15% → 0 Pt. aber Fundamentals war schon unter 50)."
---

# COST — Costco Wholesale Corporation

> **DEFCON 🟢 4 | Score 69/100 | Kein FLAG**
> Sparrate: Volle Rate | Bestandsposition

## DEFCON v3.4 Analyse (15.04.2026)

| Block | Score | Details |
|-------|-------|---------|
| Fundamentals | 29/50 | P/FCF ~53x (teuer), FCF Yield 1.88%, Fwd PE ~51x. CapEx/OCF 21.3% gut. Bilanz solide (Net Debt/EBITDA <1x). FCF $7.2B FY2025 |
| Moat | 19/20 | Wide (GuruFocus 9/10). Membership Renewal 93%, Low-Cost-Operator Pricing Power |
| Technicals | 5/10 | Moderate Distanz ATH. PT-Upside ~+15% |
| Insider | 8/10 | CEO/Insider-Käufe. Kein FLAG-Selling |
| Sentiment | 8/10 | Strong Buy-Konsens, 0% Sell |

**Screener-Exception:** GAAP-ROIC 5.6% strukturell niedrig. Membership Yield $5.3B / IC $34.9B = **15.2%** > WACC 12.3% — echter Return-Motor. Kein ROIC-FLAG.

**Nächste Aktion:** Q1 FY27 Earnings ~Dez 2026

## Sonderregel

**GM-Exception aktiv:** COST hat strukturell niedrige Gross Margin (~13%) durch Mitgliedschaftsmodell. Quick-Screener-Filter ausgenommen. Moat-Proxy stattdessen: Membership Renewal Rate >93%, Pricing Power.

## Verlinkungen

- [[DEFCON-System]]
- [[Faktortabelle-Architektur]]
- [[Analyse-Pipeline]]
- [[Tariff-Exposure-Regel]]
- [[RMS]] — gleiche Screener-Exception-Logik (hoher Return-Spread)

## Wissenschaftliche Basis
- [[5J-Fundamental-Fenster]] — 5J-Perspektive als Pflichtrahmen für alle Fundamentaldaten
- [[FCF-Primacy]] — FCF-Yield und forward P/E als primäre Bewertungsanker; trailing P/E: nur Kontext
- [[Moat-Taxonomie-Morningstar]] — Moat-Prüfung nach 8-Quellen-Schema (Wide/Narrow/None)
- [[Wissenschaftliche-Fundierung-DEFCON]] — 7-Befunde-Matrix: wissenschaftliche Validierung des DEFCON-Systems
