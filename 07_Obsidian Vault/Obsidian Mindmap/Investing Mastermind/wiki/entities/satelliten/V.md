---
tags: [satellit, aktiv, defcon-4]
ticker: V
name: Visa Inc.
sektor: Zahlungs-Infrastruktur / Netzwerk
ersatz: MA
score_aktuell: 86
defcon: 4
flag: "keins"
sparrate: "Volle Rate"
letzteAnalyse: 2026-04-15
score_valid_until: 2026-10-15
naechsterTrigger: "Q2 FY26 Earnings ~22.04.2026"
related_concepts: "[[5J-Fundamental-Fenster]], [[FCF-Primacy]], [[Moat-Taxonomie-Morningstar]]"
updated: 2026-04-16
---

# V — Visa Inc.

> **DEFCON 🟢 4 | Score 86/100 | Kein FLAG**
> Sparrate: Volle Rate | Zahlungsinfrastruktur-Monopol

## DEFCON v3.4 Analyse (15.04.2026)

| Block | Score | Details |
|-------|-------|---------|
| Fundamentals | 44/50 | CapEx/OCF ~6% (Fabless), FCF $21.6B, GM ~80%, Net Debt/EBITDA 0.31x. ROIC ~9.9% GAAP knapp < WACC ~10.5% (Goodwill Visa-Europe $19.9B) |
| Moat | 19/20 | Wide (GuruFocus 9/10). Netzwerkeffekte, Intangible Assets, Switching Costs, Efficient Scale. +1 Pricing Power (VAS +28% Q1 FY26) |
| Technicals | 7/10 | -17.6% vom ATH $375. PT-Upside +28–29%. Unter 200MA ($330) |
| Insider | 6/10 | Diskr. 90d $201K (kein FLAG). Plan-Verkäufe $24.6M (10b5-1, Lloyd Carney) |
| Sentiment | 10/10 | 37 Buy / 3 Hold / 0 Sell. Zacks 1↑ / 0↓ |

**Wide Moat:** Network Effect + Duopol mit [[MA]]. FCF-Marge ~54%, ROIC ~9.9%. Top-Score 86 gleichauf [[AVGO]].

**Nächste Aktion:** Q2 FY26 Earnings ~22.04.2026 → QuickCheck

## Stärken

- Netzwerkeffekt-Monopol: 4.1B Karten, 100M+ Händler
- Asset-Light: Kein CapEx-Problem (Fabless-Niveau ~6%)
- Kein direktes Tariff-Risiko (Infrastruktur, keine Produktion)

## Verlinkungen

- [[DEFCON-System]]
- [[Faktortabelle-Architektur]]
- [[Analyse-Pipeline]]
- [[AVGO]] — gleichauf Top-Score 86

## Wissenschaftliche Basis
- [[5J-Fundamental-Fenster]] — 5J-Perspektive als Pflichtrahmen für alle Fundamentaldaten
- [[FCF-Primacy]] — FCF-Yield und forward P/E als primäre Bewertungsanker; trailing P/E: nur Kontext
- [[Moat-Taxonomie-Morningstar]] — Moat-Prüfung nach 8-Quellen-Schema (Wide/Narrow/None)
- [[Wissenschaftliche-Fundierung-DEFCON]] — 7-Befunde-Matrix: wissenschaftliche Validierung des DEFCON-Systems
