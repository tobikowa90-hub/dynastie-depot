...
EXPECTED_FLAGS = ("--core", "--full", "--vault", "--minimal-baseline",
                  "--no-write", "--json")
...
def run(
    ...
    *,
    ...
    for flag in EXPECTED_FLAGS:
        n_checked += 1
        if flag not in text:
            severity = "warning" if flag == "--minimal-baseline" else "error"
            failures.append(FailureDetail(
                location=f"{SLASH_CMD_PATH}: flag list",
                expected=f"flag '{flag}' documented",
                actual="flag not mentioned",
                severity=severity,  # type: ignore[arg-type]
                hint=f"Slash-Doku sollte {flag} explizit nennen",
            ))
