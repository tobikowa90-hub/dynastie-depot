...
- **Datum:** 2026-09-04 (Fr, Mittag/Nachmittag) — **🔴 RICHTUNGSENTSCHEIDUNG: Spec-Track ausgesetzt, Radikalkur beschlossen (PIPELINE #83, scoring-neutral).** Session begann als Spec-Verifikation (Ziel 95 % Confidence vor Plan), endete mit dem Befund, dass genau diese Verifikation das Problem ist. **Was gebaut wurde:** `03_Tools/spec_inventory_diff.py` — mechanisches Suchraum-Komplement (Repo-Inventar minus die von der Spec genannten Dateien), deklariert seinen Suchraum selbst inkl. der Endungen, die es *nicht* ansieht. Abnahmetest: Lauf gegen Spec v1.6 (`e88f0d7`) muss `00_Core/STATE.md` reproduzieren — **der erste Entwurf fiel durch** (lenienter Stem-Match liess das blosse Wort „STATE" in einer Aufzählung als Treffer gelten; Match jetzt strikt auf Basename, endungslos nur für `.xlsx`). Commits `f147257` + `fe1ea80`. **Funde (alle verifiziert):** `00_Core/UMSTRUKTURIERUNG-2027.md` war unerkannte fünfte Regelwerk-Quelle (60/35/5, 3-Tier-Basis, „FLAG bleibt heilig — Override-frei") → **Owner-Entscheidung: zurückgezogen**, 3 live Pointer bereinigt, historische Records (Vault-log 12.06., `score_history.jsonl`) bewusst unangetastet · **Vault ist eine vierte Urteils-Quelle und driftet:** 15 Satelliten-Seiten mit `score_aktuell`/`flag` im Frontmatter, **AVGO dort 56 gegen live 59**, **COST 69 + VEEV 74 gültig bis Oktober obwohl seit Juni aus dem Roster** — Vault-Frontmatter steht in keinem §18-Sync-Set · **`05_Archiv/portfolio_returns.jsonl` + `benchmark-series.jsonl` sind aktive Append-Ziele mit 0 Treffern in der Spec** · 23 Vault-Concept-Seiten tragen `operative_regel` mit echten Scoring-Regeln · `PIPELINE.md` existiert zweimal (Core = Work-SSoT, Skill-Kopie = Analyse-Doku, kein Sync-Ziel) — in beiden Kopfzeilen + Spec §10 abgegrenzt. **Die Entscheidung:** sechs Prüfrunden fanden je eine neue Fehlerklasse **außerhalb** des Vorrunden-Suchraums — das ist ein Skalen-, kein Konvergenz-Signal. Die Komplexität sitzt nicht in der Analyse, sondern in der **Zustands-Duplikation**: dieselbe Zahl in 8 Dateien erzeugt §18 → `paragraph-18-sync` → `xlsx-smoke-test` → `system_audit` → `spec_inventory_diff`; keine dieser Schichten beantwortet eine fachliche Frage. Der Scalable-Übertrag macht die Hälfte davon **gegenstandslos statt migrationsbedürftig**. **Beschlossen: Abriss im Repo, nicht Greenfield** (unersetzlich: die vier Ledger, ING-Einstandsdaten für Entnahme 2027, fertige Analysen, Vault-Literatur). Zielbild: Broker = Live-State (nie abgeschrieben) · ein Append-only-Ledger · eine Regeldatei (eine Seite) · eine Analyse je Ticker + Vault als Literatur. **Nächster Schritt: GOOGL-Vollanalyse (Frist 22.09.) als Minimal-Lauf** — Scalable-MCP + defeatbeta + News, mitschreiben was tatsächlich gebraucht wurde; **das Protokoll dieses Laufs ist die neue Spec.** ⚠️ **Offene Owner-Frage zum Einstieg: Minimal-Lauf im Altsystem, oder erst Abriss-Schnitt festlegen?** Tier-2-Vorschlag „Verifikations-Ratsche erkennen" für `APPLIED-LEARNING.md` liegt vor, noch nicht eingetragen. Nicht gepusht.
