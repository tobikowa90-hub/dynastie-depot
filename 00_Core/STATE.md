...
- **12.05. (Di) ✅ Ruflo Runtime-Upgrade v3.6.11 → v3.7.0-alpha.21 DONE-with-Caveats — PIPELINE #50 closed** (System-Event, scoring-neutral). Pre-Doctor 7P/7W/0F, Post-Doctor 10P/7W/0F (+3 PASS: Version-Freshness flip + AIDefence-MCP + Federation-Breaker neu; +1 WARN Encryption-at-Rest als Phase-2-Defer). **Bug-A/B-Empirical-Verify DEFERRED** (Plan-Steps 4.2-4.5 + 6.4): MCP-Server PID 299 pre-Upgrade-cached, Kill für Diagnose-Reload → Connection lost, alle `mcp__ruflo__*`-Tools dieser Session offline; Session-Restart erforderlich für Bridge-Smoke-Test. `projectPath`-Parameter nicht exposed (Plan-Annahme aus PR-Body Drift gegen Reality). **Stranded-Keys-Sweep ABORTED** nach Visual-Verify-Discovery: die 51 `claude:C--Users-tobia-Code:*`-Keys sind NICHT Cross-Project-Pollution (Plan-Annahme), sondern historische Dynastie-Auto-Memory-Imports unter altem Pfad-Hash (MD-Sources unter `~/.claude/projects/C--Users-tobia-Code/memory/`). ~13 Keys ohne OneDrive-Duplikat würden bei DELETE aus DB raus + Re-Import nicht zurückbringen. Sauberer Cleanup via separate Mini-Welle MD-File-Migration. **OneDrive-Klarstellung User 12.05.:** Ordner ist NICHT Cloud-synced (weder OneDrive noch Google Drive); "OneDrive" im Pfad ist historisches Artefakt. CodeRabbit-CLI von Ubuntu-Distro nach Ubuntu-24.04 migriert; alte Distro stillgelegt (default umgesetzt). Snapshots in `05_Archiv/ruflo-doctor-history/2026-05-12-pre-upgrade.txt` + `2026-05-12-post-upgrade.txt` (Δ-Triage-Header). DB-Backups `.swarm/memory.db.pre-import-2026-05-12` + `.pre-sweep-2026-05-12` (gitignored). Detail → SYSTEM §Ruflo-Status + CORE-MEMORY §13 + git log + Vault log.md.
...
- **11.05. (Mo) spät ✅ Ruflo-Bridge-Bugs A+B UPSTREAM FIXED + PIPELINE #50 READY** — Maintainer `ruvnet` 10.05. 14:37 UTC PR #1886 commit `d0c507e6` mergt beide Fixes in `ruflo@3.7.0-alpha.21` (+ `@claude-flow/cli` + `claude-flow`, alle Dist-Tags `alpha` + `latest` + `v3alpha`). Bug A `resolveProjectMemoryDir()` mit 4 Kandidat-Hashes inkl. WSL-`/mnt/<drive>/`-Translation + neuem `projectPath`-Input-Parameter. Bug B `sanitizeMemoryKey()` an beiden `storeEntry`-Sites + Single-Source-of-Truth `DANGEROUS_KEY_CHARS`. Beide Issues CLOSED. **Lokaler Runtime v3.6.11 pinned** bis Upgrade-Welle (Werktags-Plan-Slot mit Doctor-V/N-Snapshot + Bridge-Smoke + Stranded-Keys-SQL-Sweep). PIPELINE #50 Status `DECISION-DONE+ISSUES-FILED` → **READY**.
...
*🦅 STATE.md Hub v2.1 | Dynasty-Depot | Navigation + Critical-Alert + Last-Audit | Stand: 12.05.2026 abends (PIPELINE #50 Ruflo Runtime-Upgrade v3.6.11 → v3.7.0-alpha.21 ✅ DONE-with-Caveats — Doctor 10P/7W/0F, Bug-A/B-Verify deferred auf Session-Restart, Stranded-Keys-Sweep ABORTED nach Visual-Verify-Discovery, OneDrive-Sync-Klarstellung, CodeRabbit nach Ubuntu-24.04 migriert; saubere Bug-A/B-Verify als separate Mini-Welle post-Restart)*
