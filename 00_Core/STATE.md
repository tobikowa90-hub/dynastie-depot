...
5. ~~**CLAUDE.md Routing-Refactor Tier 1**~~ **DONE 2026-04-24.** Plan `docs/superpowers/plans/2026-04-24-claude-md-routing-refactor.md` v0.2.1 vollständig ausgeführt (8 Tasks + Section-A-Follow-up). CLAUDE.md 97→86 Zeilen, 2 neue SSoT-Files: `00_Core/APPLIED-LEARNING.md` (14/20 Bullets, v2.5) + `00_Core/TOKEN-RULES.md` (Accessibility-Modell). Neue `## Routing-Table` (9 Trigger + 3 Edge-Cases) ersetzt On-Demand-Lektüre-Liste (Entscheidung A). `## Pointer (Ausgelagertes)` mit 4 Zeilen inkl. Meilensteine-Archiv (User-Entscheidung gegen Info-Verlust, AC #8 dokumentiert abweichend). Applied-Learning v2.5: +2 Bullets (Plan-Self-Review-Blindspot, Migrations-Invariante-vs-Meta-Logging). 11/11 AC PASS (2 dokumentierte Abweichungen: AC #8, AC #9b Self-Reference). Commits: `9ae0dcc`/`81829b5`/`e586d27`/`3e81a14`/`ca66785`/`2a67221`/`b99cf3b`/`1bd7492`. **Follow-ups Session 1 (heute):** Codex-Reconciliation-Pass. **Session 2:** CodeRabbit-Pass (WSL) + Vault-Update-Sweep (`07_Obsidian Vault/.../wiki/concepts/CLAUDE-md-Konstitution.md`). **Tier 2 (STATE-Split Variante B Hub) deferred** — eigene Brainstorm-Session.
...
12. **00_Core Perfect-Organization — Brainstorm-Kandidat (NEU 24.04.2026, User-Vision):** Ziel = perfekt organisierter + global verknüpfter `00_Core/`-Ordner. Umfang-Kandidaten: (a) **Tier 2b CORE-MEMORY Restrukturierung** — kalendarische Riesen-Liste (~45 §1-Einträge) → Subkategorien (nach Topic / System-Element / Zeit?) + Verknüpfung zu adressierten System-Elementen (Skills/Tools/Files). Single-File vs Split offen. Migrations-Strategie offen. (b) **Tier 2 STATE-Split Variante B Hub** (bereits in #5 Handover-Notiz deferred) — STATE.md ~10-Z-Hub + `00_Core/PORTFOLIO.md`/`PIPELINE.md`/`SYSTEM.md`. (c) **Global-Linking-Strategie** — Back-Refs zwischen 00_Core-Files systematisieren (aktuell nur per Prose-Mention). (d) **Vault-Integration** — sollen Wiki-Concepts strukturell auf neue 00_Core-Struktur Bezug nehmen? (e) **insider-intelligence/SKILL-insider-intelligence.md §215 Snapshot-First-Block-Review** (aus 24.04. Session 1 Cross-Ref-Pass): prüfen ob einzigartig oder Duplikat von TOKEN-RULES.md Snapshot-First; falls Duplikat → löschen + Cross-Ref; falls einzigartig → in TOKEN-RULES.md Skill-spezifische Ergänzungen mit §-Referenz aufnehmen. **Trigger:** Eigene Brainstorm-Session, nicht in Tier-1-Closeout mitziehen. **Gate:** Session 2 (CodeRabbit + Vault) muss erst schließen. Handover-Notiz `SESSION-HANDOVER.md` verweist hierauf.
