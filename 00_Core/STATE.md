...
12. **00_Core Perfect-Organization Tier 2 — Spec v0.4 + Plan v1.1 BEREIT, Execution offen (Session 5).** Scope-Entscheidung: **B** (a+b+c zusammen). Entscheidungs-Kaskade: G2 + H1 + R2 + V-Kompakt + L2 (Outbound-only Markdown Kopf). Skill-Versions: `dynastie-depot` v3.7.2→v3.7.3, `backtest-ready-forward-verify` 1.0.0→1.0.1. **Spec v0.4 ratified** (lokal, gitignored): 3 Advisor-Flagged-Klärungen User-resolved 24.04. Session 4 — (1a) PORTFOLIO=Primärquelle edit-direkt · (2b) Back-Ref-Scope 10 Files (4 neu + 6 Peers) · (3a) Union-Regel formal §18-Teil. **Plan v1.1** (lokal durable): `docs/superpowers/plans/2026-04-24-00core-perfect-organization.md` — 10 Tasks (Task 0 Pre-Migration-Grep/Baseline → Task 9 Review-Gates+Merge) · 103 bite-sized Steps · 1920 Z. **Codex-Pre-Execution-Review RECONCILED_WITH_FOLLOWUPS** (F1 🔴 AC #18-Gate explizit + F2 🟡 Atomarity-Drift + F3 🟡 Hard-Pause 9.7 + F4 ℹ️ Exhaustive Ticker-Reconciliation), alle inline in Plan v1.1 resolved. **Feature-Branch** `refactor/00core-perfect-organization` angelegt, noch keine Refactor-Commits. **Next:** Session 5 Execution via `superpowers:executing-plans` (~3-4h konzentrierte Session, Checkpoints nach Task 2/4/6/8/9). Post-Impl Codex-Reconciliation + CodeRabbit-Pass (Task 9.2/9.5) mandatory. **Nicht-Scope:** Vault-Concept-Seiten-Sanierung (Follow-up #13) · insider-intelligence §215-Triage.

13. **Vault-Concept-Seiten-Sanierung (NEU 24.04.2026 — Follow-up zu #12):** 8 Vault-Files referenzieren STATE.md/CORE-MEMORY.md (`wiki/concepts/STATE.md`, `wiki/concepts/CORE-MEMORY.md`, `wiki/concepts/CLAUDE-md-Konstitution.md`, `wiki/concepts/Session-Start-Protokoll.md`, `wiki/concepts/Token-Mechanik.md`, `wiki/concepts/Context-Hygiene.md`, `wiki/synthesis/Depot-State-April-2026.md`, diverse DEFCON-Concepts). Nach #12-Execution muss Vault auf neue Struktur (Hub + PORTFOLIO / PIPELINE / SYSTEM + CORE-MEMORY §12/§13) nachgezogen werden. **Trigger:** Vor nächstem Vault-heavy-Discoverability-Sweep ODER bei erstem Vault-Lint-Fail ODER User-initiierter Vault-Review — whichever first. **Risiko bei Aufschub:** Dokumentations-Drift (nicht Live-System-Break — `vault_backlinks.py` prüft Wikilinks, nicht Prose-Refs).
