...
12. **00_Core Perfect-Organization Tier 2 — Brainstorm DONE 24.04.2026 (Session 3), Spec v0.3 RATIFIZIERT.** Scope-Entscheidung: **B** (a+b+c zusammen, d+e deferred). Entscheidungs-Kaskade: G2 (4-File-Split STATE→Hub+PORTFOLIO+PIPELINE+SYSTEM) + H1 (Hub-Pure, ~25 Z mit Navigation+Critical-Alert-Slot+Last-Audit) + R2 (CORE-MEMORY §1-Auflösung, V-Kompakt = §12 Per-Ticker-Chronik + §13 System-Lifecycle-History mit `[Topic]`-Prefix, §9b löschen) + L2 (Back-Ref-Konvention Outbound-only Markdown Kopf-Sektion, kein Tooling). Skill-Versions: `dynastie-depot` v3.7.2→v3.7.3 (Patch), `backtest-ready-forward-verify` 1.0.0→1.0.1 (eigene Semver-Convention). **Spec lokal:** `docs/superpowers/specs/2026-04-24-00core-perfect-organization-design.md` (gitignored per Design) — 18 AC, Claude-Self + Codex (8 Findings / 2 Blocker / alle resolved) + Advisor (4 Punkte, 1 inline-fix PORTFOLIO Size-AC, 3 deferred-für-Plan-Session). **Feature-Branch** `refactor/00core-perfect-organization` angelegt, noch keine Commits außer Session-Close. **3 offene Klärungen für Plan-Session (Advisor-Flagged):** (1) PORTFOLIO=Primär-Layer ja/nein? (2) Back-Ref-Scope 4 neue Files vs. +Peers? (3) §18 Union-Regel formal? Details in `SESSION-HANDOVER.md` Resume-Block. **Next:** Session 4 `superpowers:writing-plans` mit Spec v0.3 → Plan in `docs/superpowers/plans/`. **Nicht-Scope:** Vault-Concept-Seiten-Sanierung (Follow-up #13 neu) · insider-intelligence §215-Triage (separater Hygiene-Task, ≤30 Min).
