...
- **18.05. (Mo) 🔴 AMZN Neuaufnahme 12. Satellit — SCORING-RELEVANT** (User-Direktive „Mein Portfolio, meine Regeln"). Score **42/🔴 D1**, **🔴 CapEx/OCF-FLAG** (TTM netto 99,2% / gross 101,7% ≫60%, schärfer als GOOGL; FCF TTM $1,2B -95%). Sparrate **0€ regelkonform** (User-Entscheidung: kein Owner-Override, FLAG heilig). Slot-Erweiterung **11→12 nenner-neutral** (FLAG=0,0 → Nenner 7,5 unverändert, alle 11 bestehenden Raten 38€/19€/0€ unverändert, Σ285€). §410 N/A (GW 2,6%). §18-Multi-Event-Sync DONE (Faktortabelle/PORTFOLIO/config.yaml/KONTEXT/CORE-MEMORY/log.md/score_history.jsonl/flag_events.jsonl/2 xlsx + §18.7-Smoke). Codex-Single-Pass (prozed. REJECT vor Sync, inhaltl. bestätigt). **OFFEN → neue Session:** Codex-Diff-Re-Review + Gemini Cross-Sync der 12 angefassten Files (User-Wunsch, Kontext-Schonung). Detail → CORE-MEMORY §12.AMZN + git log + ScoreRecord 2026-05-15_AMZN_vollanalyse.
