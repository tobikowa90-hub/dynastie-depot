...
12. **00_Core Perfect-Organization — Brainstorm-Kandidat (NEU 24.04.2026, User-Vision):** Ziel = perfekt organisierter + global verknüpfter `00_Core/`-Ordner. Umfang-Kandidaten: (a) **Tier 2b CORE-MEMORY Restrukturierung** — kalendarische Riesen-Liste (~45 §1-Einträge) → Subkategorien (nach Topic / System-Element / Zeit?) + Verknüpfung zu adressierten System-Elementen (Skills/Tools/Files). Single-File vs Split offen. Migrations-Strategie offen. (b) **Tier 2 STATE-Split Variante B Hub** (bereits in #5 Handover-Notiz deferred) — STATE.md ~10-Z-Hub + `00_Core/PORTFOLIO.md`/`PIPELINE.md`/`SYSTEM.md`. (c) **Global-Linking-Strategie** — Back-Refs zwischen 00_Core-Files systematisieren (aktuell nur per Prose-Mention). (d) **Vault-Integration** — sollen Wiki-Concepts strukturell auf neue 00_Core-Struktur Bezug nehmen? **Trigger:** Eigene Brainstorm-Session, nicht in Tier-1-Closeout mitziehen. **Gate:** Session 2 (CodeRabbit + Vault) muss erst schließen. Handover-Notiz `SESSION-HANDOVER.md` verweist hierauf.
